    ...
    class MinHeap{
        ...
        void insert(int data){
            if(root == null){ // if tree empty
                root = new Node(data);
                tail = root; // last node is current
            }
            else if(tail.left == null){ // if last node left empty
                tail.left = new Node(data); // set to left node of last node
        ...
        void minHeapify(Node node){
            if(node.parent != null){ // if not root
                if(node.parent.data > node.data){ // if current is smaller than parent then swap
        ...
        void revMinHeapify(Node node){
            // if node is empty and left is empty that means tree is blank
            if(node == null || node.left == null)
                return;
            Node min = node.left; // left is set min
            if(node.right != null && min.data > node.right.data){ // get min of left and right if right present
                min = node.right;
            }
            // check if child is smaller than parent, if yes then swap
